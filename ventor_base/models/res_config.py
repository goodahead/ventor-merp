...
class VentorConfigSettings(models.TransientModel):
    ...
    def _set_show_packages_fields(self, previous_group):
        ...
        if group_stock_tracking_lot != self.group_stock_tracking_lot:
            operation_type_ids.show_packages_fields = self.group_stock_tracking_lot
            if not self.group_stock_tracking_lot:
                operation_type_ids.show_put_in_pack_button = self.group_stock_tracking_lot
                operation_type_ids.force_destination_package_scan = self.group_stock_tracking_lot

    def _set_show_product_owner_field(self, previous_group):
        operation_type_ids = self.env['stock.picking.type'].search([])
        ...
        if (
            ...
            operation_type_ids.show_product_owner_field = self.group_stock_tracking_owner
    ...
    def set_values(self):
        ...
        self.sudo()._set_show_packages_fields(previous_group)
        self.sudo()._set_show_product_owner_field(previous_group)
        return res
